...
export const formatGraphQLError = (error) => {
  ...
  // In production, only hide truly internal/sensitive errors
  if (process.env.NODE_ENV === 'production') {
    // List of keywords that indicate internal errors that shouldn't be exposed
    ...
    // Check if error message contains any internal error keywords
    const isInternalError = internalErrorKeywords.some(keyword => 
      error.message.includes(keyword) || originalError?.message?.includes(keyword)
    );

    // Also hide errors that are not operational and are 500-level errors
    const isNonOperationalServerError = 
      originalError && 
      !originalError.isOperational && 
      (originalError.statusCode === undefined || originalError.statusCode >= 500);

    // Hide truly internal errors
    if (isInternalError || isNonOperationalServerError) {
      return {
  ...
  // Return formatted error with message visible (validation errors, auth errors, etc.)
  return {
    message: error.message,
    extensions: {
